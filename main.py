...
def extract_title(md_text: str) -> str:
    ...
    match = re.search(r"^---$(?P<yaml>.*?)^---$", md_text, flags=re.MULTILINE | re.DOTALL)
...
def make_dataframe(root_dir: Path) -> pd.DataFrame:
    ...
    path_series = pd.Series(map(lambda p: str(p.relative_to(root_dir)), paths), dtype="string", name="Path")
    title_series = pd.Series(map(lambda p: extract_title(p.read_text()), paths), dtype="string", name="Title")
    ...
    assignee_series = pd.Series(pd.NA, dtype="string", index=path_series.index, name="Assignee")

    return pd.concat([path_series, title_series, rag_series, assignee_series], axis="columns")
...
if __name__ == "__main__":
    ...
    parser.add_argument("root_dir", type=Path, help="Root directory to recursively search for Markdown documents")
    parser.add_argument("output_csv", type=Path, default=None, nargs='?', help="Optional path for output to CSV file (if not provided, output to stdout)")
